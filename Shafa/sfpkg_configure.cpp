...
namespace shafa {
    void sfpkg_configure::pkg_configure_hub(toml::table& sfContTable)
    {
        if (sfContTable.contains("settings"))
        {
            ...
            if (it != compilation_table.end())
            {
                ...
                m_pkgConfigSetup->projectSettings->projectName = string_to_wstring(it->second.as_string()->get());
            }
            else
            ...
            if (it != compilation_table.end())
            {
                ...
                m_pkgConfigSetup->projectSettings->projectVersion = string_to_wstring(it->second.as_string()->get());
            }
            else
            ...
            if (it != compilation_table.end())
            {
                try
                {
                    m_pkgConfigSetup->projectSettings->projectType = sfProjectTypeHelper::to_enum(string_to_wstring(it->second.as_string()->get()));
                }
            ...
            for (auto& [key, value] : compilation_table)
            {
                if (key.str() == "cppCompilerPath")
                {
                    ...
                    if (value.as_string()->get() == "auto")
                    {
                        LOG_DEBUG(L"Auto C++ compiler search is enabled.")
                            compilerPath = foreman_finder_for_pkg();

                        m_pkgConfigSetup->compilationList->cppCompilerPath = compilerPath;
                        LOG_DEBUG(L"Found C++ compiler: " + m_pkgConfigSetup->compilationList->cppCompilerPath.wstring());

                        *value.as_string() = m_pkgConfigSetup->compilationList->cppCompilerPath.string();

                        std::ofstream file(m_pkgConfigSetup->configFilePath);
                        file << sfContTable;
                        file.close();
                    }
                    else
                        m_pkgConfigSetup->compilationList->cppCompilerPath = string_to_wstring(value.as_string()->get());
                ...
                else if (key.str() == "cppLinkerPath")
                {
                    ...
                    if (value.as_string()->get() == "auto")
                    {
                        ...
                        linkerPath = foreman_finder_for_pkg(true);

                        m_pkgConfigSetup->compilationList->cppLinkerPath = linkerPath;
                        LOG_DEBUG(L"Found C++ linker: " + m_pkgConfigSetup->compilationList->cppLinkerPath.wstring());

                        *value.as_string() = m_pkgConfigSetup->compilationList->cppLinkerPath.string();
                        ...
                        m_pkgConfigSetup->compilationList->cppLinkerPath = string_to_wstring(value.as_string()->get());
                ...
                else if (key.str() == "cppLibLinkerPath")
                {
                    ...
                    if (value.as_string()->get() == "auto")
                    {
                        ...
                        libLinkerPath = foreman_finder_for_pkg(true);

                        m_pkgConfigSetup->compilationList->cppLibLinkerPath = libLinkerPath;
                        LOG_DEBUG(L"Found C++ lib linker: " + m_pkgConfigSetup->compilationList->cppLibLinkerPath.wstring());

                        *value.as_string() = m_pkgConfigSetup->compilationList->cppLibLinkerPath.string();
                        ...
                        m_pkgConfigSetup->compilationList->cppLibLinkerPath = string_to_wstring(value.as_string()->get());
                ...
                else if (key.str() == "cppCompiler" && !(value.as_string()->get() == "clang"))
                {
                    ...
                    m_pkgConfigSetup->compilationList->cppCompiler = sfCppCompilersHelper::to_enum(string_to_wstring(value.as_string()->get()));
                }
                else if (key.str() == "cppVersion" && !(value.as_string()->get() == "c++20"))
                {
                    ...
                    m_pkgConfigSetup->compilationList->cppVersion = sfCppVersionsHelper::to_enum(string_to_wstring(value.as_string()->get()));
                }
                else if (key.str() == "debugFlags")
                {
                    m_pkgConfigSetup->compilationList->debugFlags.clear();
                    ...
                    {
                        m_pkgConfigSetup->compilationList->debugFlags += L" " + string_to_wstring(flag.as_string()->get());
                    }

                    LOG_INFO(L"Non default debug flags are set to: " + m_pkgConfigSetup->compilationList->debugFlags);
                }
                else if (key.str() == "releaseFlags")
                {
					m_pkgConfigSetup->compilationList->releaseFlags.clear();
                    if (!value.as_array()->is_homogeneous(toml::node_type::string))
                    {
                    ...
                    for (const auto& flag : *value.as_array())
                    {
                        m_pkgConfigSetup->compilationList->releaseFlags += L" " + string_to_wstring(flag.as_string()->get());
                    }

                    LOG_INFO(L"Non default release flags are set to: " + m_pkgConfigSetup->compilationList->releaseFlags);
                }
                else if (key.str() == "libDirs")
                {
                    ...
                    {
                        m_pkgConfigSetup->compilerArgs->cppLibDirs.push_back(string_to_wstring(folder.as_string()->get()));
                    }
                ...
                else if (key.str() == "libs")
                {
                    ...
                    {
                        m_pkgConfigSetup->compilerArgs->cppLibs.push_back(string_to_wstring(lib.as_string()->get()));
                    }
                ...
                else if (key.str() == "includeDirs")
                {
                    ...
                    for (const auto& folder : *value.as_array())
                    {
                        m_pkgConfigSetup->compilerArgs->cppIncludeDirs.push_back(string_to_wstring(folder.as_string()->get()));
                    }
                ...
                else if (key.str() == "projectBuildType" && !(value.as_string()->get() == "debug"))
                {
                    ...
                    m_pkgConfigSetup->compilationList->projectBuildType = sfProjectBuildTypeHelper::to_enum(string_to_wstring(value.as_string()->get()));
                ...
                else if (key.str() == "packages")
                {
                    ...
                    {
                        m_pkgConfigSetup->compilerArgs->packages.push_back(string_to_wstring(package.as_string()->get()));
                    }
                }
            }
        }
        ...
        if (sfContTable.contains("configure")) {
            ...
            for (const auto& [key, value] : settings_table) {
                ...
                else if (key.str() == "buildFolder" && !(value.as_string()->get() == "Build"))
                {
                    ...
                    m_pkgConfigSetup->configList->buildFolder = string_to_wstring(value.as_string()->get());
                }
                else if (key.str() == "outputReleaseFolder" && !(value.as_string()->get() == "Output/Release"))
                {
                    ...
                    m_pkgConfigSetup->configList->outputReleaseFolder = string_to_wstring(value.as_string()->get());
                }
                else if (key.str() == "outputDebugFolder" && !(value.as_string()->get() == "Output/Debug"))
                {
                    ...
                    m_pkgConfigSetup->configList->outputDebugFolder = string_to_wstring(value.as_string()->get());
                }
            }
        }
        else {
    ...
    void sfpkg_configure::pkg_configuration_construct()
    {
        try
        {
            if (!std::filesystem::exists(m_pkgConfigSetup->configList->buildFolder))
            {
                LOG_INFO(L"Build folder not found. Creating build folder.");
                std::filesystem::create_directories(m_pkgConfigSetup->configList->buildFolder);
            }
            if (!std::filesystem::exists(m_pkgConfigSetup->configList->outputDebugFolder))
            {
                LOG_INFO(L"Debug output folder not found. Creating debug output folder.");
                std::filesystem::create_directories(m_pkgConfigSetup->configList->outputDebugFolder);
            }
            if (!std::filesystem::exists(m_pkgConfigSetup->configList->outputReleaseFolder))
            {
                LOG_INFO(L"Release output folder not found. Creating release output folder.");
                std::filesystem::create_directories(m_pkgConfigSetup->configList->outputReleaseFolder);
            }

            write_identifiers_to_file_for_pkg();
    ...
    void sfpkg_configure::write_identifiers_to_file_for_pkg()
    {
        LOG_DEBUG(L"Log path: " + m_pkgConfigSetup->configList->cacheFilePath.wstring());
        if (!std::filesystem::exists(m_pkgConfigSetup->configList->cacheFilePath)) {
            LOG_INFO(L"Data.info file not found. Creating data.info file.");
        }
        else
            std::wofstream(m_pkgConfigSetup->configList->cacheFilePath, std::wios::out | std::wios::trunc).close();
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(std::filesystem::current_path()))
        {
            ...
            {
                cppFileIdentity.push_back(std::make_pair(entry.path().wstring(), file_hashing_for_pkg(entry.path().wstring())));
            }
        }

        try
        {
            std::wofstream log_file(m_pkgConfigSetup->configList->cacheFilePath);
            if (log_file.is_open()) {
    ...
    std::filesystem::path sfpkg_configure::foreman_finder_for_pkg(bool isLinker)
    {
        ...
        std::wstring label{ m_pkgConfigSetup->projectSettings->projectName + L'-' + m_pkgConfigSetup->projectSettings->projectVersion };
        if (!isLinker)
            switch (m_pkgConfigSetup->compilationList->cppCompiler)
            {
            ...
            if (m_pkgConfigSetup->projectSettings->projectType != sfProjectType::staticLibrary) {
